...
namespace SyntheseImage
{
    public class Box
    {
        ...
        public bool RayIntersect(Ray ray)
        {
            ...
            // X slab Max box size
            Double tx1 = (pMin.X - ray.origin.X) * rinvx;
            Double tx2 = (pMax.X - ray.origin.X) * rinvx;

            Double tminX = Math.Min(tx1, tx2);
            Double tmaxX = Math.Max(tx1, tx2);

            // Y slab
            Double ty1 = (pMin.Y - ray.origin.Y) * rinvy;
            Double ty2 = (pMax.Y - ray.origin.Y) * rinvy;

            Double tminY = Math.Max(tminX, (Math.Min(ty1, ty2)));
            Double tmaxY = Math.Min(tmaxX, (Math.Max(ty1, ty2)));

            // Z slab
            Double tz1 = (pMin.Z - ray.origin.Z) * rinvz;
            Double tz2 = (pMax.Z - ray.origin.Z) * rinvz;
