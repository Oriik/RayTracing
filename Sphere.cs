...
namespace SyntheseImage
{
    public class Sphere : Shape
    {
        ...
        public override float RayIntersect(Rayon ray, out Shape returnShape)
        {
            ...
            float B = 2 * (Vector3.Dot(ray.origine, ray.direction) - Vector3.Dot(m_center, ray.direction));
            float C = Vector3.Dot(Vector3.Subtract(m_center, ray.origine), Vector3.Subtract(m_center, ray.origine)) - (m_radius * m_radius);
            float D = B * B - 4 * A * C;
            if (D < 0)
                return -1.0f;
            else
